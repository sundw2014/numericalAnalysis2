...
hp_Number mpf_power(hp_Number x, unsigned int n)
{
	mpf_class powval(1);
    mpf_class mpf_x = x.getValue();
    for (unsigned int num = n; num != 0; --num)
	{
        powval *= mpf_x;
	}
	return hp_Number(powval);
}

hp_Number ln_Taylor(double x)
{
    hp_Number e(std::string(E));
    hp_Number t = hp_Number(x)/mpf_power(e.getValue(),5);
    // hp_Number t("0.5");
    hp_Number result(0);
